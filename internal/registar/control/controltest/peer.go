...
func main() {
	...
	tlsConfig, err := newTLSConfig(caTLSCert, udpAddr)
	if err != nil {
		slog.Error("create TLS config", "err", err)
		os.Exit(1)
	}

	connector := control.NewConnector(transport, tlsConfig)
...
func newTLSConfig(ca tls.Certificate, addr *net.UDPAddr) (*tls.Config, error) {
	privkey, err := cert.NewPrivateKey()
	if err != nil {
		return nil, fmt.Errorf("new private key: %w", err)
	}
	peerCert, err := cert.NewIPCert(ca.Leaf, ca.PrivateKey, addr.IP, privkey.Public())
	if err != nil {
